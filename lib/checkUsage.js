...
export async function getUserPlan(supabase, userId) {
  ...
  try {
    const stripePlan = await getActiveStripeSubscriptionPlan(supabase, normalizedUserId)
    if (stripePlan) {
      console.log('[checkUsage:getUserPlan] using Stripe plan', {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle()
    console.log('[checkUsage:getUserPlan] fallback user_usage query', {
      userId: normalizedUserId,
