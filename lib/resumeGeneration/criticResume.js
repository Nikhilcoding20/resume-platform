...
/** Re-instate quantified bullets the critic dropped, without exceeding per-role caps. */
  ...
  out.experience = revExp.map((job, i) => {
    ...
    for (const metricBullet of metricBullets) {
      ...
      if (bullets.length >= maxCount) {
        ...
        if (weakIdx >= 0) bullets[weakIdx] = metricBullet
        continue
      }
 ...
 * Page fitting runs in the API route AFTER this function returns.
 */
