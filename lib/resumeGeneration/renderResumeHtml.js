...
export function buildReplacements(document, template = '', { skipFit = false } = {}) {
  ...
  const summaryBlock = fitted.summary
    ? `<div class="resume-summary-block resume-section-block resume-section"${atsMb ? ` style="margin-bottom:${atsMb};"` : ''}>${sectionHeading(t === 'ats' ? 'Professional Summary' : 'Summary', t, fitAdjustments)}<p style="margin:0;font-size:${bodyPt};line-height:${bodyLh};color:#000;font-family:${t === 'ats' ? ATS_FONT : 'inherit'};">${escapeHtml(fitted.summary)}</p></div>`
    : ''
