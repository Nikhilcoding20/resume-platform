...
export function selectBestBullets(bullets, maxCount, minCount = MIN_BULLETS_PER_ROLE) {
  const deduped = flattenAndDedupeBullets(bullets)
...
function estimateContentLines(document) {
  ...
  lines += skillGroupsFromDocument(document).reduce((sum, g) => sum + 1 + Math.ceil((g.skills?.length || 0) / 4), 0)
  return Math.round(lines)
}

function trimSkillsForTemplate(doc, template, maxCategories, maxPerCategory) {
  const groups = skillGroupsFromDocument(doc)
  if (!groups.length) return doc
  doc.skills = Object.fromEntries(
    groups.slice(0, maxCategories).map((g) => [g.category, g.skills.slice(0, maxPerCategory)])
  )
  return doc
}
...
export function fitContentForTemplate(document, template) {
  ...
  // Step 3: Trim skills — ATS: max 4 categories × 5 skills; others: slightly reduced.
  if (t === 'ats') {
    trimSkillsForTemplate(doc, t, 4, 5)
  } else {
    trimSkillsForTemplate(doc, t, 5, 8)
  }
  ...
  // Step 5: Tighter ATS skills if still overflowing.
  if (t === 'ats') {
    trimSkillsForTemplate(doc, t, 3, 4)
    if (estimateContentLines(doc) <= limit) return doc
  }

  // Step 6: Last resort — reduce font size by 0.5pt (minimum 10pt body).
...
function sectionHeading(title, template) {
  ...
  if (t === 'ats') {
    ...
    return `<h2 class="resume-section-heading ats-heading" style="margin:14pt 0 0 0;font-size:11pt;font-weight:bold;text-transform:none;letter-spacing:0;color:#000;line-height:1.3;">${escapeHtml(displayTitle)}</h2><hr class="ats-section-rule" style="border:none;border-top:0.5pt solid #000;margin:4pt 0 10pt 0;width:100%;height:0;" />`
  }
...
function formatBulletsAts(bullets, fitAdjustments = null) {
  ...
  const items = flattenAndDedupeBullets(bullets)
    .map(
      (b) =>
        `<div class="ats-bullet-row" style="display:flex;align-items:flex-start;gap:0.4em;margin:0 0 2pt 0;padding-left:0.15in;line-height:${lh};font-size:${bodyPt}pt;font-family:Georgia,Garamond,serif;">
    ...
    .join('')
  if (!items) return ''
  return `<div class="resume-bullets ats-bullets" style="margin:3pt 0 0 0;">${items}</div>`
}
...
function formatSkillsAts(groups) {
  ...
  return groups
    .map((g) => {
      ...
      return `<p class="skill-group resume-section" style="margin:0 0 5pt 0;font-size:10.5pt;line-height:1.3;font-family:Georgia,Garamond,serif;"><strong>${label}:</strong> ${items}</p>`
    })
    .join('')
}
...
export function formatEducationHtml(items, template = '') {
  const t = template?.toLowerCase()
  const fs = t === 'minimal' ? '10.5pt' : t === 'ats' ? '10.5pt' : '10pt'
...
function formatContactLine(document, template) {
  ...
  if (t === 'ats') {
    const parts = []
    if (c.location?.trim()) parts.push(escapeHtml(c.location.trim()))
    if (c.email?.trim()) parts.push(escapeHtml(c.email.trim()))
    if (c.phone?.trim()) parts.push(escapeHtml(c.phone.trim()))
    if (linkedin) parts.push(escapeHtml(linkedin))
    else if (portfolio) {
      parts.push(escapeHtml(portfolio.replace(/^https?:\/\//i, '').replace(/^www\./i, '')))
    }
    return { line: parts.join(' &#8226; '), extraLine: '' }
  }
...
export function getPdfStyles(template = '') {
  ...
  if (t === 'ats') {
    templateRules = `
  body.tpl-ats { font-family: Georgia, Garamond, serif !important; color: #000 !important; font-size: 10.5pt; line-height: 1.25; }
  body.tpl-ats .resume-header { text-align: center !important; }
  ...
  body.tpl-ats .resume-contact { text-align: center !important; font-size: 10.5pt !important; }
  body.tpl-ats .resume-section-block { margin-bottom: 14pt !important; }
  body.tpl-ats .ats-section-rule { border: none !important; border-top: 0.5pt solid #000 !important; margin: 4pt 0 10pt 0 !important; }
  body.tpl-ats .resume-bullets.ats-bullets { margin: 3pt 0 0 0 !important; }
  body.tpl-ats .ats-bullet-row { display: flex !important; align-items: flex-start !important; margin-bottom: 2pt !important; padding-left: 0.15in !important; }
...
export function buildReplacements(document, template = '') {
  ...
  const fontDelta = fitAdjustments?.fontSizeDelta || 0
  const bodyPt = Math.max(10, (t === 'minimal' || t === 'ats' ? 10.5 : 10) + fontDelta)
  const bodyLh = t === 'minimal' ? '1.45' : fitAdjustments?.lineHeight || (t === 'ats' ? '1.25' : '1.35')
