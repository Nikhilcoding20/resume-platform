...
function formatBulletsModern(bullets) {
  const items = (bullets || [])
    .map(
      (b) =>
        `<p class="modern-bullet-row" style="margin:0 0 2pt 0;padding-left:10pt;text-indent:-10pt;line-height:1.3;font-size:10pt;color:#1a1a1a;font-family:${MODERN_FONT};"><span style="display:inline-block;width:10pt;margin-left:-10pt;">&#8226;</span> ${escapeHtml(b)}</p>`
    )
    .join('')
  if (!items) return ''
...
export function getPdfStyles(template = '', fitAdjustments = null) {
  ...
  body.tpl-modern td.modern-sidebar-cell { width: 30% !important; vertical-align: top !important; background: #f4f5f8 !important; padding: 20px !important; }
  body.tpl-modern td.modern-main-cell { width: 70% !important; vertical-align: top !important; background: #ffffff !important; padding: 20px !important; }
  ...
  body.tpl-modern .modern-bullet-row { font-size: 10pt !important; color: #1a1a1a !important; }`
  } else if (t === 'minimal') {
    templateRules = `
