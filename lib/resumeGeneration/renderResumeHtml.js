...
/** @page margin for Minimal: 0.6in top/bottom, 0.7in left/right */
export const MINIMAL_PDF_PAGE_MARGIN = '0.6in 0.7in'
...
const MINIMAL_SECTION_GAP = '14pt'
const MINIMAL_JOB_GAP = '10pt'
const MINIMAL_BULLET_GAP = '3pt'
...
export function getPdfMarginIn(template = '', fitAdjustments = null) {
  ...
  if (t === 'minimal') {
    return MINIMAL_PDF_PAGE_MARGIN
  }
...
function minimalSectionHeader(title, { first = false } = {}) {
  const marginTop = first ? '0' : MINIMAL_SECTION_GAP
  return `<div class="minimal-section-head" style="margin:${marginTop} 0 ${MINIMAL_SECTION_GAP} 0;">
    <p style="margin:0 0 4pt 0;font-size:9pt;font-weight:700;text-transform:uppercase;letter-spacing:normal;color:${MINIMAL_COLOR_HEADER};font-family:${MINIMAL_FONT};line-height:1.3;">${escapeHtml(String(title).toUpperCase())}</p>
...
function formatMinimalHeader(document) {
  const name = escapeHtml(document.name || '')
  ...
  if (contact) {
    parts.push(
      `<p style="margin:0 0 8pt 0;font-size:9pt;line-height:1.45;color:${MINIMAL_COLOR_MUTED};font-family:${MINIMAL_FONT};text-align:left;">${contact}</p>`
    )
  }
  parts.push(minimalHorizontalRule({ color: MINIMAL_COLOR_RULE, weight: '1px', margin: '0' }))
  return `<header class="minimal-header" style="margin:0 0 ${MINIMAL_SECTION_GAP} 0;">${parts.join('')}</header>`
}

function formatMinimalBullets(bullets) {
  const list = bullets || []
  if (!list.length) return ''
  return list
    .map((b, i) => {
      const isLast = i === list.length - 1
      const mb = isLast ? MINIMAL_JOB_GAP : MINIMAL_BULLET_GAP
      return `<p style="margin:0 0 ${mb} 0;padding-left:10px;text-indent:-10px;font-size:9.5pt;line-height:1.45;color:${MINIMAL_COLOR_BODY};font-family:${MINIMAL_FONT};"><span style="display:inline-block;width:10px;margin-left:-10px;font-size:9.5pt;">&#8226;</span> ${escapeHtml(b)}</p>`
    })
    .join('')
}

function formatMinimalExperience(experience) {
  if (!experience?.length) return ''
  return experience
    .map((job) => {
      const company = escapeHtml(job.company || '')
      ...
      return `<div class="experience-item minimal-job" style="margin-bottom:${MINIMAL_JOB_GAP};page-break-inside:avoid;font-family:${MINIMAL_FONT};">
        <div style="${rowStyle}">
        ...
        <p style="margin:0 0 4pt 0;font-size:10pt;font-style:italic;color:${MINIMAL_COLOR_TITLE};text-align:left;line-height:1.45;font-family:${MINIMAL_FONT};">${title || '&#160;'}</p>
        ${formatMinimalBullets(job.bullets)}
      </div>`
    })
    .join('')
}

function formatMinimalSkills(groups) {
  if (!groups?.length) return ''
  return groups
    .map((g) => {
      const label = escapeHtml(g.category || 'Skills')
      const items = (g.skills || []).map((s) => escapeHtml(String(s))).join(', ')
      return `<p class="minimal-skill-line" style="margin:0 0 6pt 0;font-size:9.5pt;line-height:1.45;color:${MINIMAL_COLOR_BODY};font-family:${MINIMAL_FONT};"><span style="font-weight:700;color:${MINIMAL_COLOR_BODY};">${label}:</span> ${items}</p>`
    })
    .join('')
}

function formatMinimalEducation(items) {
  if (!Array.isArray(items) || items.length === 0) return ''
  const rowStyle =
  ...
  return items
    .map((e) => {
      ...
      const inst = escapeHtml(e.institution || '')
      const year = escapeHtml(String(e.graduationYear || '').trim())
      return `<div class="education-item minimal-edu" style="margin-bottom:10pt;page-break-inside:avoid;font-family:${MINIMAL_FONT};">
        <div style="${rowStyle}">
          <span style="font-size:9.5pt;font-weight:700;color:${MINIMAL_COLOR_COMPANY};text-align:left;flex:1;min-width:0;">${degree || '&#160;'}</span>
          ${year ? `<span style="font-size:9.5pt;color:${MINIMAL_COLOR_MUTED};text-align:right;white-space:nowrap;flex-shrink:0;">${year}</span>` : ''}
        </div>
        <p style="margin:2pt 0 0 0;font-size:9.5pt;font-style:italic;color:${MINIMAL_COLOR_MUTED};line-height:1.45;font-family:${MINIMAL_FONT};">${inst || '&#160;'}</p>
      </div>`
    })
    .join('')
}

function formatMinimalCertifications(items) {
  if (!Array.isArray(items) || items.length === 0) return ''
  return items
    .map((c) => {
      const name = escapeHtml(c.name || '')
      ...
      return `<p class="minimal-cert" style="margin:0 0 6pt 0;font-size:9.5pt;line-height:1.45;color:${MINIMAL_COLOR_BODY};font-family:${MINIMAL_FONT};">${parts.join(' · ')}</p>`
    })
    .join('')
}

/** Full Minimal template body (header + sections), inline styles only. */
export function formatMinimalResume(document, skillGroups) {
  const sections = []
  let sectionIndex = 0
  const nextFirst = () => sectionIndex++ === 0

  sections.push(formatMinimalHeader(document))

  if (document.summary) {
    sections.push(
      minimalSectionHeader('Professional Summary', { first: nextFirst() }),
      `<p style="margin:0 0 ${MINIMAL_SECTION_GAP} 0;font-size:9.5pt;line-height:1.45;color:${MINIMAL_COLOR_BODY};font-family:${MINIMAL_FONT};">${escapeHtml(document.summary)}</p>`
    )
  }

  const experienceHtml = formatMinimalExperience(document.experience)
  if (experienceHtml) {
    sections.push(
      minimalSectionHeader('Experience', { first: nextFirst() }),
      `<div class="minimal-experience-block" style="margin:0 0 ${MINIMAL_SECTION_GAP} 0;">${experienceHtml}</div>`
    )
  }

  const educationHtml = formatMinimalEducation(document.education)
  if (educationHtml) {
    sections.push(
      minimalSectionHeader('Education', { first: nextFirst() }),
      `<div class="minimal-education-block" style="margin:0 0 ${MINIMAL_SECTION_GAP} 0;">${educationHtml}</div>`
    )
  }

  const certsHtml = formatMinimalCertifications(document.certifications)
  if (certsHtml) {
    sections.push(
      minimalSectionHeader('Certifications', { first: nextFirst() }),
      `<div class="minimal-certs-block" style="margin:0 0 ${MINIMAL_SECTION_GAP} 0;">${certsHtml}</div>`
    )
  }

  const skillsHtml = formatMinimalSkills(skillGroups)
  if (skillsHtml) {
    sections.push(
      minimalSectionHeader('Skills', { first: nextFirst() }),
...
export function getPdfStyles(template = '', fitAdjustments = null) {
  ...
  body.tpl-modern td:first-child { height: 100% !important; min-height: 11in !important; vertical-align: top !important; background-color: ${MODERN_SLATE} !important; }`
    ...
    templateRules = `
  body.tpl-minimal { font-family: ${MINIMAL_FONT} !important; font-size: 9.5pt !important; line-height: 1.45 !important; color: ${MINIMAL_COLOR_BODY} !important; background: #ffffff !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
...
export function buildReplacements(document, template = '', { skipFit = false } = {}) {
  ...
  return {
    ...
    '{{minimal_content}}': t === 'minimal' ? formatMinimalResume(fitted, groups) : '',
