...
function estimateContentLines(document, template = '') {
  ...
  lines += Math.ceil(String(document.summary || '').length / 95)
  ...
  lines += skillGroupsFromDocument(document).reduce(
    (sum, g) => sum + 1 + Math.ceil((g.skills?.length || 0) / (t === 'ats' ? 5 : 4)),
    0
  )
  return Math.round(lines)
}

/** Trim skills within categories only — never drop whole categories. */
function trimSkillsPerCategory(doc, maxPerCategory) {
...
function formatContactLine(document, template) {
  ...
  if (t === 'ats') {
    ...
    const fontPt = rawLine.length > 78 ? 9.5 : 10.5
...
export function getPdfStyles(template = '') {
  ...
  body.tpl-ats .ats-bullet-row { display: flex !important; align-items: flex-start !important; margin-bottom: 1.5pt !important; padding-left: 0.15in !important; }
...
export function buildReplacements(document, template = '') {
  ...
  return {
    ...
    '{{contact_line}}':
      t === 'ats'
        ? `<span style="font-size:${contact.fontPt || 10.5}pt;line-height:1.25;white-space:nowrap;">${contact.line}</span>`
        : contact.line,
