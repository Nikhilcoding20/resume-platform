...
export function fitContentForTemplate(document, template) {
  ...
  let passes = 0
  while (estimateContentLines(doc) > limit && passes < 6) {
    passes += 1
    if (passes === 1) {
      doc.experience = (doc.experience || []).map((job, i) => ({
        ...job,
        bullets: (job.bullets || []).slice(0, i >= 2 ? 3 : 4),
      }))
    } else if (passes === 2) {
      const sentences = String(doc.summary || '')
        .split(/(?<=[.!?])\s+/)
        .filter(Boolean)
      doc.summary = sentences.slice(0, 2).join(' ')
    } else if (passes === 3) {
      doc.experience = (doc.experience || []).map((job, i) => ({
        ...job,
        bullets: (job.bullets || []).slice(0, i >= 1 ? 2 : 3),
      }))
    } else if (passes === 4) {
      doc.experience = (doc.experience || []).slice(0, 4)
    } else if (passes === 5) {
      doc.certifications = (doc.certifications || []).slice(0, 3)
      const groups = skillGroupsFromDocument(doc)
      doc.skills = Object.fromEntries(
        groups.slice(0, 4).map((g) => [g.category, g.skills.slice(0, 6)])
      )
    } else {
      break
    }
...
function formatBulletsAts(bullets) {
  const items = (bullets || [])
    .map((b) => `<li style="margin-bottom:3pt;line-height:1.25;font-size:10.5pt;">${escapeHtml(b)}</li>`)
    .join('')
  if (!items) return ''
  return `<ul class="resume-bullets" style="margin:2pt 0 0 0;padding-left:0.25in;list-style-type:disc;list-style-position:outside;">${items}</ul>`
}
  ...
  return experience
    .map((job) => {
      ...
      return `<div class="experience-item resume-section" style="margin-bottom:7pt;page-break-inside:avoid;">
        <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin:0 0 1pt 0;font-size:10.5pt;line-height:1.25;">
          <span style="font-weight:bold;flex:1;min-width:0;">${company || '&#160;'}</span>
          ${loc ? `<span style="flex-shrink:0;text-align:right;">${loc}</span>` : ''}
        </div>
        <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin:0 0 2pt 0;font-size:10.5pt;line-height:1.25;">
          <span style="font-style:italic;flex:1;min-width:0;">${title || '&#160;'}</span>
          ${dates ? `<span style="flex-shrink:0;text-align:right;">${dates}</span>` : ''}
        </div>
        ${formatBulletsAts(job.bullets)}
      </div>`
    })
    .join('')
}
...
export function formatExperienceHtml(experience, template = '') {
  ...
  return formatExperienceAts(experience)
}
...
export function getPdfStyles(template = '') {
  ...
  `
  if (t === 'ats') {
    return `${base}<style>body.tpl-ats{font-family:Georgia,Garamond,serif!important;color:#000!important;font-size:10.5pt;line-height:1.25;}body.tpl-ats h1{font-size:17pt;font-weight:bold;}body.tpl-ats .resume-bullets li{margin-bottom:3pt!important;}</style>`
  }
  if (t === 'modern') {
...
export function buildReplacements(document, template = '') {
  ...
  const summaryBlock = fitted.summary
    ? `<div class="resume-summary-block resume-section">${sectionHeading(t === 'ats' ? 'Professional Summary' : 'Summary', t)}<p style="margin:0;font-size:${t === 'minimal' ? '10.5pt' : '10pt'};line-height:${t === 'minimal' ? '1.45' : '1.35'};color:#1a1a1a;">${escapeHtml(fitted.summary)}</p></div>`
    : ''
  ...
  return {
    ...
    '{{experience}}': formatExperienceHtml(fitted.experience, template),
