...
function sectionHeading(title, template, fitAdjustments = null) {
  ...
  if (t === 'modern') {
    return `<h2 class="resume-section-heading modern-heading" style="margin:0 0 6pt 0;font-size:10.5pt;font-weight:700;text-transform:uppercase;color:#1e3a5f;border-bottom:2px solid #1e3a5f;padding-bottom:3px;font-family:${MODERN_FONT};">${escapeHtml(title)}</h2>`
  }
  if (t === 'creative') {
...
function formatBulletsModern(bullets) {
  const items = (bullets || [])
    .map(
      (b) =>
        `<li style="margin-bottom:2pt;line-height:1.3;font-size:10pt;color:#1a1a1a;font-family:${MODERN_FONT};">${escapeHtml(b)}</li>`
    )
    .join('')
  if (!items) return ''
  return `<ul class="resume-bullets" style="margin:2pt 0 0 0;padding-left:14pt;list-style-type:disc;">${items}</ul>`
}
  ...
  return groups
    .map((g) => {
      ...
      const items = (g.skills || [])
        .map(
          (s) =>
            `<p style="margin:0 0 2pt 0;font-size:9.5pt;line-height:1.35;color:#e2e8f0;font-family:${MODERN_FONT};">${escapeHtml(s)}</p>`
        )
        .join('')
      return `<div class="skill-group resume-section" style="margin-bottom:8pt;font-family:${MODERN_FONT};">
        <p style="margin:0 0 4pt 0;font-size:9pt;font-weight:700;text-transform:uppercase;color:#ffffff;">${cat}</p>
        ${items}
      </div>`
    })
    .join('')
    ...
    return items
      .map((e) => {
        ...
        return `<div class="education-item resume-section" style="margin-bottom:8pt;font-family:${MODERN_FONT};">
          <p style="margin:0 0 2pt 0;font-size:10pt;font-weight:700;color:#1e3a5f;line-height:1.25;">${inst || '&#160;'}</p>
...
export function formatCertificationsHtml(items, template = '', sidebar = false, fitAdjustments = null) {
  ...
  return items
    .map((c) => {
      ...
      if (sidebar) {
        return `<p class="cert-item resume-section" style="margin:0 0 ${mb} 0;font-size:${fs};line-height:1.35;color:#e2e8f0;font-family:${MODERN_FONT};">${name}${year ? ` (${year})` : ''}</p>`
      }
...
function formatSidebarContact(document) {
  const c = document.contact || {}
  const blocks = []
  const pStyle = `margin:0 0 5pt 0;font-size:9.5pt;line-height:1.35;color:#e2e8f0;font-family:${MODERN_FONT};`
  if (c.email?.trim()) {
  ...
  if (c.portfolio?.trim()) {
    ...
    blocks.push(`<p style="margin:0;font-size:9.5pt;line-height:1.35;color:#e2e8f0;font-family:${MODERN_FONT};">${escapeHtml(p)}</p>`)
  }
  return blocks.join('')
}
...
export function getPdfStyles(template = '', fitAdjustments = null) {
  ...
  body.tpl-modern td.modern-sidebar-cell { width: 30% !important; vertical-align: top !important; background: #1e3a5f !important; color: #ffffff !important; padding: 16pt 12pt !important; }
  body.tpl-modern td.modern-main-cell { width: 70% !important; vertical-align: top !important; background: #ffffff !important; padding: 16pt 14pt !important; }
  body.tpl-modern h1.resume-name { font-size: 22pt !important; color: #ffffff !important; margin: 0 0 12pt 0 !important; }
  body.tpl-modern h2.modern-heading { font-size: 10.5pt !important; color: #1e3a5f !important; border-bottom: 2px solid #1e3a5f !important; }`
  } else if (t === 'minimal') {
    templateRules = `
