...
/** Only trim when estimated lines exceed this fraction of page capacity (95%, not 85%). */
const TRIM_START_RATIO = 0.95
...
function applyRoleBulletLimits(experience, template = '') {
  return (experience || []).map((job, i) => ({
    ...job,
    bullets: selectBestBullets(
      job.bullets,
      bulletsForRoleIndex(i, template),
      template?.toLowerCase() === 'ats' ? 4 : MIN_BULLETS_PER_ROLE
    ),
  }))
}

function applyRoleBulletLimitsReduced(experience, template = '') {
  const caps = template?.toLowerCase() === 'ats' ? [4, 4, 3] : [4, 3, 3]
  return (experience || []).map((job, i) => ({
    ...job,
    bullets: selectBestBullets(job.bullets, caps[Math.min(i, caps.length - 1)], MIN_BULLETS_PER_ROLE),
  }))
}
...
/** Progressive ATS skills trim when over page budget (keep all categories at 6 until trim needed). */
function fitAtsSkillsToBudget(doc, limit) {
  if (estimateContentLines(doc, 'ats') <= limit) return doc

  trimSkillsPerCategory(doc, 5)
  if (estimateContentLines(doc, 'ats') <= limit) return doc

  trimSkillsPerCategory(doc, 4)
  if (estimateContentLines(doc, 'ats') <= limit) return doc

  trimSkillsPerCategory(doc, 3)
  if (estimateContentLines(doc, 'ats') <= limit) return doc

  while (estimateContentLines(doc, 'ats') > limit && skillGroupsFromDocument(doc).length > 1) {
...
export function fitContentForTemplate(document, template) {
  ...
  if (t === 'ats') {
    fitAtsSkillsToBudget(doc, limit)
    if (estimateContentLines(doc, template) <= limit) return doc
  } else {
  ...
  doc.experience = applyRoleBulletLimitsReduced(doc.experience, template)
  if (estimateContentLines(doc, template) <= limit) return doc
...
function sectionHeading(title, template) {
  ...
  if (t === 'ats') {
    const displayTitle = title.toLowerCase() === 'skills' ? 'SKILLS' : title
    return `<h2 class="resume-section-heading ats-heading" style="margin:10pt 0 0 0;font-size:11pt;font-weight:bold;text-transform:none;letter-spacing:0;color:#000;line-height:1.3;">${escapeHtml(displayTitle)}</h2><hr class="ats-section-rule" style="border:none;border-top:0.5pt solid #000;margin:0 0 4pt 0;width:100%;height:0;" />`
  }
...
function formatBulletsAts(bullets, fitAdjustments = null) {
  ...
  const lh = fitAdjustments?.lineHeight || 1.4
  const items = strictDedupeBullets(bullets, (b) => bulletQualityScore(b))
    .map(
      (b) =>
        `<div class="ats-bullet-row" style="display:flex;align-items:flex-start;gap:0.4em;margin:0 0 1.5pt 0;padding-left:0.15in;line-height:${lh};font-size:${bodyPt}pt;font-family:Georgia,Garamond,serif;">
...
function formatExperienceAts(experience, fitAdjustments = null) {
  ...
  return experience
    .map((job) => {
      ...
      return `<div class="experience-item resume-section" style="margin-bottom:8pt;page-break-inside:avoid;font-family:Georgia,Garamond,serif;">
        <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin:0 0 1pt 0;font-size:${bodyPt};line-height:1.25;">
          ...
          ${loc ? `<span style="flex-shrink:0;text-align:right;font-weight:normal;">${loc}</span>` : ''}
        </div>
        <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin:0 0 3pt 0;font-size:${bodyPt};line-height:1.25;">
          <span style="font-style:italic;flex:1;min-width:0;">${title || '&#160;'}</span>
          ${dates ? `<span style="flex-shrink:0;text-align:right;font-style:italic;">${dates}</span>` : ''}
        </div>
...
function formatSkillsAts(groups) {
  ...
  return groups
    .map((g) => {
      ...
      return `<p class="skill-group resume-section" style="margin:0 0 4pt 0;font-size:10pt;line-height:1.25;font-family:Georgia,Garamond,serif;"><strong>${label}:</strong> ${items}</p>`
    })
    .join('')
}
...
export function formatEducationHtml(items, template = '') {
  const t = template?.toLowerCase()
  const fs = t === 'minimal' ? '10.5pt' : t === 'ats' ? '10pt' : '10pt'
  const mb = t === 'minimal' ? '12pt' : t === 'ats' ? '8pt' : '6pt'
  const lh = t === 'minimal' ? '1.45' : '1.3'
  const fontFamily = t === 'ats' ? 'font-family:Georgia,Garamond,serif;' : ''
...
function formatContactLine(document, template) {
  ...
  if (t === 'ats') {
    ...
    const fontPt = rawLine.length > 78 ? 9.5 : 10
    return { line, extraLine: '', fontPt, nowrap: true }
  }
...
export function getPdfStyles(template = '') {
  ...
  if (t === 'ats') {
    templateRules = `
  body.tpl-ats { font-family: Georgia, Garamond, serif !important; color: #000 !important; font-size: 10pt; line-height: 1.25; }
  body.tpl-ats .resume-header { text-align: center !important; margin-bottom: 10pt !important; }
  body.tpl-ats h1.resume-name { font-size: 24pt !important; font-weight: bold !important; text-align: center !important; font-family: Georgia, Garamond, serif !important; margin: 0 0 6pt 0 !important; }
  body.tpl-ats .resume-contact { text-align: center !important; white-space: nowrap !important; margin: 0 !important; }
  body.tpl-ats .resume-section-block { margin-bottom: 10pt !important; }
  body.tpl-ats .ats-section-rule { border: none !important; border-top: 0.5pt solid #000 !important; margin: 0 0 4pt 0 !important; }
  body.tpl-ats .experience-item { margin-bottom: 8pt !important; }
  body.tpl-ats .resume-bullets.ats-bullets { margin: 2pt 0 0 0 !important; }
  body.tpl-ats .ats-bullet-row { display: flex !important; align-items: flex-start !important; margin-bottom: 1.5pt !important; padding-left: 0.15in !important; font-size: 10pt !important; line-height: 1.4 !important; }
  body.tpl-ats .skill-group { font-size: 10pt !important; line-height: 1.25 !important; }
...
export function buildReplacements(document, template = '') {
  ...
  const bodyLh = t === 'minimal' ? '1.45' : fitAdjustments?.lineHeight || (t === 'ats' ? '1.4' : '1.35')

  const summaryBlock = fitted.summary
    ? `<div class="resume-summary-block resume-section-block resume-section">${sectionHeading(t === 'ats' ? 'Professional Summary' : 'Summary', t)}<p style="margin:0;font-size:${bodyPt};line-height:${bodyLh};color:#000;font-family:${t === 'ats' ? 'Georgia,Garamond,serif' : 'inherit'};">${escapeHtml(fitted.summary)}</p></div>`
    : ''

  const experienceHtml = formatExperienceHtml(fitted.experience, template, fitAdjustments)
  const experienceBlock =
    t === 'ats' && experienceHtml
      ? `<div class="resume-experience-block resume-section-block resume-section">${sectionHeading('Professional Experience', template)}${experienceHtml}</div>`
      : experienceHtml
  ...
  return {
    ...
    '{{contact_line}}':
      t === 'ats'
        ? `<span style="font-size:${contact.fontPt || 10}pt;line-height:1.25;white-space:nowrap;">${contact.line}</span>`
        : contact.line,
