...
function atsBodyPt(fitAdjustments = null) {
  ...
  return Math.max(9, ATS_BODY_PT + fontDelta)
}

function atsNamePt(fitAdjustments = null) {
  const fontDelta = fitAdjustments?.fontSizeDelta || 0
  return Math.max(22, ATS_NAME_PT + fontDelta)
}

/** Resolved ATS layout for HTML + PDF (optional post-fit fill spacing). */
export function resolveAtsLayout(fitAdjustments = null) {
  const fill = Boolean(fitAdjustments?.fillPage)
  return {
    bodyPt: atsBodyPt(fitAdjustments),
    namePt: atsNamePt(fitAdjustments),
    lineHeight: fitAdjustments?.lineHeight || (fill ? ATS_BULLET_LINE_HEIGHT_FILL : ATS_BULLET_LINE_HEIGHT),
    sectionSpacing: fitAdjustments?.sectionSpacing || (fill ? ATS_SECTION_SPACING_FILL : ATS_SECTION_SPACING),
    jobBlockSpacing: fitAdjustments?.jobBlockSpacing || (fill ? ATS_JOB_BLOCK_SPACING_FILL : ATS_JOB_BLOCK_SPACING),
  }
}
...
const ATS_MAX_SKILL_CATEGORIES = 3
const ATS_MAX_SKILLS_PER_CATEGORY = 4
const ATS_MAX_CERTIFICATIONS = 3
const ATS_SUMMARY_TRIM_SENTENCES = 2
...
function trimCertifications(doc, maxCount = ATS_MAX_CERTIFICATIONS) {
  if (!Array.isArray(doc.certifications)) return doc
  doc.certifications = doc.certifications.slice(0, maxCount)
  return doc
}

/** Re-apply ATS caps after normalize (skills/certs can grow from profile merge). */
export function applyAtsContentCaps(doc) {
  return enforceAtsHardLimits(doc)
}

/** Step 1: dedupe, bullet caps, skills, certifications (summary unchanged). */
function enforceAtsHardLimits(doc) {
  ...
  trimCertifications(doc, ATS_MAX_CERTIFICATIONS)
  return doc
}

function applyAtsFillIfRoom(doc, limit) {
  const lines = estimateContentLines(doc, 'ats')
  if (lines > limit) return doc
  if (doc._fitAdjustments?.fontSizeDelta < 0) return doc
  if (limit - lines < ATS_FILL_MIN_SLACK) return doc

  doc._fitAdjustments = {
    ...(doc._fitAdjustments || {}),
    fillPage: true,
    lineHeight: ATS_BULLET_LINE_HEIGHT_FILL,
    sectionSpacing: ATS_SECTION_SPACING_FILL,
    jobBlockSpacing: ATS_JOB_BLOCK_SPACING_FILL,
  }
  return doc
}

/**
 * ATS one-page fit order: dedupe → 4/4/3 bullets → 3×4 skills → 3 certs → 2-sentence summary → −0.5pt font → fill spacing.
 */
function fitAtsContentToOnePage(doc, limit) {
  enforceAtsHardLimits(doc)
  if (estimateContentLines(doc, 'ats') <= limit) {
    return applyAtsFillIfRoom(doc, limit)
  }

  doc.summary = trimSummaryToSentences(doc.summary, ATS_SUMMARY_TRIM_SENTENCES)
  if (estimateContentLines(doc, 'ats') <= limit) {
    return applyAtsFillIfRoom(doc, limit)
  }
...
function estimateContentLines(document, template = '') {
  ...
  const fill = t === 'ats' && document._fitAdjustments?.fillPage
  const bulletFactor = t === 'ats' ? (fill ? 1.22 : 1.12) : 1.2
  ...
  for (const job of document.experience || []) {
    lines += t === 'ats' ? (fill ? 2.8 : 2.5) : 3
    lines += (job.bullets || []).length * bulletFactor
  }
  lines += (document.education || []).length * (t === 'ats' ? 1.8 : 2)
  const certCount = Math.min((document.certifications || []).length, t === 'ats' ? ATS_MAX_CERTIFICATIONS : 99)
  lines += certCount * (t === 'ats' ? 1 : 1)
  if (t === 'ats') {
...
export function formatProjectsBlock(projects, template = '', fitAdjustments = null) {
  ...
  return `<div class="resume-projects-block resume-section-block resume-section" style="margin-bottom:${mb};">${sectionHeading('Projects', template)}${inner}</div>`
}
...
export function formatEducationBlock(items, template = '', fitAdjustments = null) {
  ...
  return `<div class="resume-education-block resume-section${extraClass}"${mbStyle}>${sectionHeading('Education', template)}${inner}</div>`
}

export function formatCertificationsHtml(items, template = '', sidebar = false) {
  const t = template?.toLowerCase()
  const fs = sidebar ? '9.5pt' : t === 'ats' ? `${ATS_BODY_PT}pt` : '10pt'
  const mb = sidebar ? '4pt' : t === 'ats' ? '3pt' : '6pt'
  if (!Array.isArray(items) || items.length === 0) return ''
  return items
    .map((c) => {
...
export function formatCertificationsBlock(items, template = '', fitAdjustments = null) {
  ...
  const extraClass = t === 'ats' ? ' resume-section-block' : ''
  const mbStyle = t === 'ats' ? ` style="margin-bottom:${atsSectionBlockMargin(fitAdjustments)};"` : ''
  return `<div class="resume-certifications-block resume-section${extraClass}"${mbStyle}>${sectionHeading('Certifications', template)}${inner}</div>`
}
...
export function getPdfStyles(template = '', fitAdjustments = null) {
  ...
  body.tpl-ats .ats-bullet-row { display: flex !important; align-items: flex-start !important; margin-bottom: ${ATS_BULLET_GAP} !important; padding-left: 0.12in !important; font-size: ${layout.bodyPt}pt !important; line-height: ${layout.lineHeight} !important; }
  body.tpl-ats .skill-group { font-size: ${layout.bodyPt}pt !important; line-height: 1.35 !important; }
...
export function buildReplacements(document, template = '', { skipFit = false } = {}) {
  ...
  const summaryBlock = fitted.summary
    ? `<div class="resume-summary-block resume-section-block resume-section"${atsMb ? ` style="margin-bottom:${atsMb};"` : ''}>${sectionHeading(t === 'ats' ? 'Professional Summary' : 'Summary', t)}<p style="margin:0;font-size:${bodyPt};line-height:${bodyLh};color:#000;font-family:${t === 'ats' ? ATS_FONT : 'inherit'};">${escapeHtml(fitted.summary)}</p></div>`
    : ''

  const experienceHtml = formatExperienceHtml(fitted.experience, template, fitAdjustments)
  const experienceBlock =
    t === 'ats' && experienceHtml
      ? `<div class="resume-experience-block resume-section-block resume-section" style="margin-bottom:${atsMb};">${sectionHeading('Experience', template)}${experienceHtml}</div>`
      : experienceHtml
  ...
  return {
    ...
    '{{skills_block}}':
      t === 'modern' || !groups.length
        ? ''
        : t === 'ats'
          ? `<div class="resume-skills-block resume-section-block resume-section" style="margin-bottom:${atsMb};">${sectionHeading('Skills', template)}${formatSkillGroupsHtml(groups, template)}</div>`
...
export function normalizeClientResumeContentForRender(clientContent, profile, template = '') {
  ...
  if (template?.toLowerCase() === 'ats') {
    ...
    document._fitAdjustments = fitAdjustments
  }
