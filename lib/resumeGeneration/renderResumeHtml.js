...
function modernSidebarSectionHeader(title, { gapBefore = false } = {}) {
  ...
  return `<p style="margin:${top} 0 6px 0;font-size:8pt;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#ffffff;font-family:${MODERN_FONT};">${escapeHtml(title)}</p>`
}

function modernMainSectionHeader(title, { gapBefore = false } = {}) {
  const top = gapBefore ? '14px' : '0'
  return `<p style="margin:${top} 0 8px 0;padding:0 0 4px 0;font-size:10pt;font-weight:700;color:${MODERN_SLATE};border-bottom:1px solid ${MODERN_SLATE};font-family:${MODERN_FONT};line-height:1.3;">${escapeHtml(title)}</p>`
}
...
export function formatModernSidebar(document, groups) {
  ...
  const parts = [
    `<p style="margin:0 0 10px 0;font-size:20pt;font-weight:700;color:#ffffff;line-height:1.2;font-family:${MODERN_FONT};word-wrap:normal;white-space:normal;">${name}</p>`,
...
export function getPdfStyles(template = '', fitAdjustments = null) {
  ...
  body.tpl-ats .ats-bullet-marker { flex-shrink: 0 !important; }`
    ...
    templateRules = `
  body.tpl-modern { margin: 0 !important; padding: 0 !important; font-family: ${MODERN_FONT} !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  body.tpl-modern table { border-collapse: collapse !important; table-layout: fixed !important; }`
  } else if (t === 'minimal') {
    templateRules = `
...
export function buildReplacements(document, template = '', { skipFit = false } = {}) {
  ...
  return {
    ...
    '{{education_block}}': t === 'modern' ? '' : formatEducationBlock(fitted.education, template, fitAdjustments),
