...
/** First N words normalized — used to catch paraphrased duplicate bullets. */
export function bulletFirstWordsKey(text, wordCount = 8) {
  const words = normalizeBulletKey(text).split(/\s+/).filter(Boolean)
  return words.slice(0, wordCount).join(' ')
}

function bulletQualityScoreForDedupe(text) {
  let score = 0
  if (/\d+\s*%|\$\s?\d|\d+\s*[xX]\b|\b\d{1,3}(,\d{3})+\+?\b/.test(String(text || ''))) score += 10
  return score + Math.min(wordCount(text), 20)
}

function bulletWordList(text) {
  return normalizeBulletKey(text).split(/\s+/).filter(Boolean)
}

/** True if two bullets share minLen consecutive words in order. */
export function bulletsShareConsecutiveWords(a, b, minLen = 6) {
  const wa = bulletWordList(a)
  const wb = bulletWordList(b)
  if (wa.length < minLen || wb.length < minLen) return false
  const wbJoined = wb.join(' ')
  for (let i = 0; i <= wa.length - minLen; i++) {
    const slice = wa.slice(i, i + minLen).join(' ')
    if (wbJoined.includes(slice)) return true
  }
  return false
}

/** Drop near-duplicates: 6+ shared consecutive words — keep stronger/longer bullet. */
export function dedupeConsecutiveWordOverlap(bullets, minWords = 6, scoreFn = bulletQualityScoreForDedupe) {
  const items = (bullets || [])
    .map((b, i) => ({
      b: String(b).trim(),
      i,
      score: scoreFn(b),
      len: bulletWordList(b).length,
    }))
    .filter((x) => x.b)
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (let j = i + 1; j < items.length; j++) {
      ...
      if (!bulletsShareConsecutiveWords(items[i].b, items[j].b, minWords)) continue
      const keepI =
        items[i].score > items[j].score ||
        (items[i].score === items[j].score && items[i].len >= items[j].len)
      removed.add(keepI ? j : i)
      if (!keepI) break
    }
  }
  return items.filter((_, idx) => !removed.has(idx)).sort((a, b) => a.i - b.i).map((x) => x.b)
}

/** Flatten newline-embedded bullets, strip markers, dedupe exact + near-duplicate overlaps. */
export function flattenAndDedupeBullets(bullets) {
  ...
  return dedupeConsecutiveWordOverlap(strictDedupeBullets(flat), 6)
}

/** Remove bullets whose first 8 words match another in the same list (keep stronger/metric version). */
export function strictDedupeBullets(bullets, scoreFn = bulletQualityScoreForDedupe) {
  const list = (bullets || []).map((b, i) => ({
    b: String(b).trim(),
    i,
    key: bulletFirstWordsKey(b, 8),
    score: scoreFn(b),
  })).filter((x) => x.b && x.key)
  ...
  for (const item of list) {
    ...
    if (!prev || item.score > prev.score || (item.score === prev.score && item.i < prev.i)) {
      bestByKey.set(item.key, item)
    }
  }
  return [...bestByKey.values()].sort((a, b) => a.i - b.i).map((x) => x.b)
}

export function dedupeExperienceBullets(experience) {
  return (experience || []).map((job) => ({
    ...job,
    bullets: flattenAndDedupeBullets(job.bullets || []),
  }))
}
...
function normalizeExperienceEntry(raw, index) {
  ...
  bullets = strictDedupeBullets(bullets.map((b) => sanitizeBullet(b)).filter(Boolean)).slice(0, 4)
...
function normalizeProjectEntry(raw) {
  ...
  const bullets = desc
    ? desc
        ...
        .filter(Boolean)
    : []
  return { name, bullets: bullets.slice(0, 4) }
}

function projectsFromProfile(profile) {
  let arr = profile?.projects
  ...
  return arr.map(normalizeProjectEntry).filter(Boolean).slice(0, 3)
}

function enforceSummarySentences(summary, maxSentences = 4) {
...
export function normalizeResumeDocument(parsed, profile) {
  ...
  const document = {
    ...
    education,
    certifications,
    projects: projectsFromProfile(profile),
