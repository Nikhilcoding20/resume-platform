...
function normalizeExperienceEntry(raw, index) {
  ...
  bullets = bullets.map((b) => sanitizeBullet(b)).filter(Boolean).slice(0, 5)
...
function enforceSummaryTwoSentences(summary) {
...
export function normalizeResumeDocument(parsed, profile) {
  ...
  const document = {
    name,
    contact,
    summary: enforceSummaryTwoSentences(parsed?.summary),
    experience,
    education,
    certifications,
