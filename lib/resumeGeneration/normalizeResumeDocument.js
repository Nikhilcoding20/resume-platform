...
/** Flatten newline-embedded bullets, strip markers, dedupe exact + first-8-word matches. */
export function flattenAndDedupeBullets(bullets) {
  ...
  return strictDedupeBullets(flat)
}
...
export function dedupeExperienceBullets(experience) {
  return (experience || []).map((job) => ({
    ...job,
    bullets: strictDedupeBullets(job.bullets || []),
  }))
}
