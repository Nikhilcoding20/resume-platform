...
export function brandedEmailHtml({ title, intro, rows, cta }) {
  const rowHtml = rows
    .map(
      (r) => `
      <tr>
  ...
  const ctaHtml =
    cta && typeof cta.href === 'string' && cta.href.startsWith('http') && typeof cta.label === 'string'
      ? `
          <tr>
            <td colspan="2" style="padding:24px 0 4px;text-align:center;border-top:1px solid #eaeaf2;">
              <a href="${escapeHtml(cta.href)}" style="display:inline-block;padding:14px 28px;background:linear-gradient(135deg,#6366f1 0%,#7c3aed 45%,#06b6d4 100%);color:#ffffff;text-decoration:none;font-weight:700;border-radius:12px;font-size:15px;box-shadow:0 4px 14px rgba(99,102,241,0.35);">${escapeHtml(cta.label)}</a>
            </td>
          </tr>`
      : ''
        ...
        <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #eaeaf2;box-shadow:0 4px 24px rgba(15,23,42,0.06);">
          ...
          <tr>
            <td style="padding:24px 28px 28px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">${rowHtml}${ctaHtml}</table>
            </td>
          </tr>
          <tr>
